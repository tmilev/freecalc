...
[(\fcArrows) () ne  [\fcGetColorCode{\fcColorLine}] \fcLineWidth [\fcDashes] (\fcLineStyle)]
}
...
\define@key{fcGraphics}{dashes}[\renewcommand{\fcDashes}{[1 1] 0}]{\renewcommand{\fcDashes}{#1}}
...
\rput(! #2\space #3\space add 2 div dup cos #4\space 0.2 add mul exch sin #4\space 0.2 add mul){#5}%
}
...
/plotArrowHeadVirtual {\fcArrowPlot} def
/setdashVirtual {setdash} def
}

\newcommand{\fcSetUpGraphicsToFileAndScreen}{
graphicsFile ( /plotArrowHeadVirtual {\fcArrowPlot} def ) writestring
...
strokeVirtual %
useArrows{[] 0 setdashVirtual [pointLeft 0 get \fcCoordsIIIdToPS] [pointRight 0 get \fcCoordsIIIdToPS] plotArrowHeadVirtual}if %
}if %
end %
}
...
/forceForeground patchOptions 3 get def%
...
stroke
exch [ exch \fcCoordsIIIdToPS ] exch [exch \fcCoordsIIIdToPS ] plotArrowHeadVirtual
}
...
thePatch \fcPatchGetNormal \fcScreen\space pop \fcVectorScalarVector  dup == (the goddamned fucking crap piece of shit) == 0 gt
{ thePatch \fcPatchGetColorVU setrgbcolorVirtual (i'm fucking here!) == }
{ thePatch \fcPatchGetColorUV setrgbcolorVirtual (i'm at the other fukcing place) ==}
ifelse
0.5 setalpha
newpathVirtual
