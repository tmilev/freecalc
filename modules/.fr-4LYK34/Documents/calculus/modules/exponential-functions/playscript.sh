...
rm supertempfile
#LECTURENAME=`sed -n -e '/.*\\subtitle{[a-zA-Z]* [0-9]*}.*$/p' $1 | sed -e 's/.*subtitle{[a-zA-Z]* \([0-9]*\)}.*$/\1/g'`   # why doesn't this work?
...
do
	...
	(
		...
		mkdir ${MODULE}
		mkdir ${MODULE}/pictures
		sed -e "s/{pictures/{`echo "${PWD##*/}"`\/${MODULE}\/pictures/g" $1 | sed -n -e "/begin module[ ]*$MODULE[ ]*$/,/end module[ ]*$MODULE[ ]*$/p" > ${MODULE}/${MODULE}.tex
		echo "\\input{../../modules/${PWD##*/}/${MODULE}/${MODULE}}" >> ~/Documents/calculus/lectures/test01/test01.tex
