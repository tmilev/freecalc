...
\uncover<5->{
\begin{proof}
$z \bar z= \uncover<6->{(x+iy)(x-iy)=}\uncover<7->{x^2\alert<8>{-}(\alert<8>{i}y)^{\alert<8>{2}}=} \uncover<8->{x^2\alert<8>{+}y^2}$ \uncover<8->{is real and non-negative.} \uncover<9->{ $z \bar z=0$ implies $x^2+y^2=0$,} \uncover<10->{which implies $x=y=0$.}
\end{proof}
}
\uncover<11->{
\begin{definition}
The quantity $\alert<11,13>{|z|}=\sqrt{z\bar z}= \sqrt{ {\alert<12>{x}}^2+ { \alert<12>{y }}^2}$ is called the \alert<11>{\emph{absolute} value of $z$}.
\end{definition}
}
\end{frame}