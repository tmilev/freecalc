...
\rput[bl](0.6,1){$\frac{z}{|z|} \uncover<10->{=\alert<10>{\cos \theta} + i\alert<11>{\sin \theta}}$}%
}%
\uncover<handout:0|5>{%
...
\alert<12,15>{$e^{i\theta}=\cos \theta+i\sin \theta$}.
\end{theorem}
} 
\begin{lemma}
...
\item<9-> Then $\alert<14>{\frac{z}{|z|}= \alert<12>{ \alert<10>{ \cos \theta} +i \alert<11>{\sin \theta}}} \uncover<12->{ \alert<12>{= e^{i\theta}}} $.
\uncover<13->{
\begin{definition}[Polar form of complex numbers]
Let $z\neq 0$. $\alert<14>{z=|z|(\alert<15>{\cos\theta+i\sin \theta)}}= |z| \alert<15>{ e^{i\theta}} $ is called polar form of $z $.
\end{definition}
}
\item<16-> Let $\alert<19>{\rho = \ln \alert<17>{|z|} } \uncover<17->{ = \ln \alert<17>{\sqrt{ x^2+y^2} }}  \uncover<18->{ = \frac{1}{2}\ln(x^2+y^2)}$.
\item<19-> Then $z=\alert<19>{|z|} ( \cos\theta+ i\sin\theta) = \alert<19>{ e^{\rho}}( \cos \theta +i\sin \theta) \uncover<20->{ =e^{\rho} e^{i\theta } }\uncover<21->{=e^{\rho + i \theta}.} $
\end{itemize}
\end{frame}
...
%\item<7-> Euler's formula implies: $\alert<8>{z= ( \cos \theta +i\sin \theta)= e^{\rho} e^{i\theta}}$.
\end{itemize}
\end{frame}