...
\rput[bl](0.6,1){$\frac{z}{|z|} \uncover<10->{=\cos \theta+ i\sin \theta}$}%
}%
\uncover<handout:0|5>{%
...
\alert<12>{$e^{i\theta}=\cos \theta+i\sin \theta$}.
\end{theorem}
} 
\begin{lemma}
...
\item<9-> Then $\alert<15>{\frac{z}{|z|}= \alert<12>{ \alert<10>{ \cos \theta} +i \alert<11>{\sin \theta}}} \uncover<12->{ \alert<12>{= e^{i\theta}}} $.
\item<13-> Let $\alert<16>{\rho = \ln \alert<14>{|z|} } \uncover<14->{ = \ln \alert<14>{\sqrt{ x^2+y^2} }}  \uncover<15->{= \frac{1}{2}\ln(x^2+y^2)}$.
\item<16-> Then $\alert<16>{z=\alert<17>{|z|}( \cos\theta+ i\sin\theta) }\uncover<17->{= \alert<17>{ e^{\rho}}( \cos \theta +i\sin \theta)} \uncover<18->{ =e^{\rho} e^{i\theta } } = \uncover<19->{e^{\rho +i\theta}}$
\end{itemize}
\uncover<20->{
