...
\begin{enumerate}
\item  \alertNoH{2}{If $\ds \int_a^\infty f(x) \diff x$ is convergent}, then \alertNoH{3}{$\ds \int_a^\infty g(x)\diff x$ is convergent}.
\item \alertNoH{4}{If $\ds \int_a^\infty g(x) \diff x$ is divergent}, \alertNoH{5}{then $\ds \int_a^\infty f(x)\diff x$ is divergent.}
\end{enumerate}
\end{theorem}
\begin{columns}[c]
\column{.4\textwidth}
