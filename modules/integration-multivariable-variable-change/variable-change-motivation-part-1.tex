...
\begin{itemize}
\item Let $\fcv e_1,\dots, \fcv e_n$ be basis vectors. \uncover<2->{Fix point $\fcv y=(y_1,\dots, y_n)$.}
\item<3-> Let $\Delta y_1,\dots, \Delta y_n$ be small numbers. \uncover<4->{Construct small box $B$ with corner $\fcv y$ spanned by the vectors $\Delta y_1 \fcv e_1,\dots, \Delta y_n \fcv e_n$.} 
\item<5-> The point $\fcv y$ and the corners $\fcv y +\Delta y_1 \fcv e_1,\dots, \fcv y+\Delta y_n\fcv e_n$ suffice to identify $B$. %Mark those corners.
\item<6-> $\Vol (B)=\Delta y_1 \dots \Delta y_n$. 

\item<7-> Let the image of $B$ be $\fcv f(B)={\color{blue}C}$. ${\color{blue}C} $ is a ``curvilinear box''. 
\item<8-> Let ${\color{green}E}$ be the parrallelotope at $\fcv f(\fcv y)$ spanned by images of the corners of $B$. \uncover<9,10->{\alert<10>{Then $\Vol ({\color{blue}C})\approx \Vol_n ({\color{green}E})$.}}
\end{itemize}
