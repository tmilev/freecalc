...
\solution{\ref{problemintegralxydxdyovery=x^2+1,y=2x^2-x-1}

We start by plotting the region. $ y=x^2+1$ is a parabola symmetric across the $y$ axis; $y=2x^2-x-1$ is a parabola with vertex at $x=\frac{ 1}{4} $, $y=-\frac{9}{8}$. The two parabolas intersect when 
...
\end{array}
\]
Thus the region looks as plotted below.

\begin{pspicture}
\pscustom*[linecolor=\fcColorAreaUnderGraph]{
\psplot{-1}{2}{x x mul 1 add}
\psplot{2}{-1}{x x mul 2 mul x sub 1 sub}
}
\psplot[linecolor=\fcColorGraph]{-1}{2}{x x mul 1 add}
\psplot[linecolor=\fcColorGraph]{2}{-1}{x x mul 2 mul x sub 1 sub}
\fcAxesStandardNoFrame{-2}{-1.5}{3}{5}
\fcLabels{3}{5}
\end{pspicture}

Therefore our region $\mathcal R$ is parametrized as 
\[
\mathcal R=\{(x,y)| 2x^2-x-1 \leq x^2+1 \}.
\]
This is a single curvilinear trapezoid. We can integrate directly as follows.

\[
\begin{array}{rcl}
\displaystyle \iint_{\mathcal R} xy \diff x \diff y&=& \displaystyle  \int_{x=-1}^{x=2}\int_{y=2x^2-x-1}^{y=x^2+1}x y \diff y \diff x \\
&=&\displaystyle \int_{x=-1}^{x=2} \left[\frac{1}{2} x y^{2} \right]_{y=2x^2-x-1}^{y=x^2+1}  \diff y\\
&=&\displaystyle \int_{x=-1}^{x=2} \left(-\frac{3}{2} x^{5}+2 x^{4}+\frac{5}{2} x^{3}- x^{2} \right)\diff x \\
&=&\displaystyle  \left[ -\frac{1}{4} x^{6}+\frac{2}{5} x^{5}+\frac{5}{8} x^{4}-\frac{1}{3} x^{3} \right]_{x=-1}^{x=2} \\
&=&\displaystyle \frac{153}{40}\quad. 
\end{array}
\]
}