...
\only<16->{
So far we proved: \alert<22>{for any $\varepsilon >0$ there exists $\delta>0$ so that for all $h<\delta$ we have $\left| \frac{\int_{x }^{x+h}f(t)dt }{h}-f(x)\right|<\varepsilon$}.

\[
\begin{array}{l}
\uncover<17->{\displaystyle G'(x)=\lim\limits_{h\to 0}\frac{ \alert<19>{ G(x+h)} -\alert<20>{G(x)}}{h} }\\ 
\uncover<18->{\displaystyle \phantom{ G'(x) }= \lim\limits_{h\to 0}  \frac{ \alert<19>{ \alert<21>{\int_{a}^{x+h}} f(t)dt} - \alert<20>{ \alert<21>{\int_{a}^{x}} f(t)dt}}{h} }\\
\uncover<21->{\displaystyle \phantom{ G'(x) }= \alert<22>{\lim\limits_{h\to 0}\frac{\alert<21>{\int_{x}^{x+h}}f(t)dt }{h} \uncover<22->{=f(x)}}}
\end{array}
\]
