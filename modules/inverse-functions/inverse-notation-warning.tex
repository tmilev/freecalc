...
\end{quotation}
-Bjarne Stoustroup, creator of the programming language C++
}
...
\alert<13>{\text{should not be used } } & \alert<14>{ \text{when } n\neq -1, 1,2,3,\dots .}
\end{array} \right.
\]
}
\uncover<15->{ To reduce confusion, if possible, use $\frac{1}{f(x)}$ instead of $\left(f(x)\right)^{-1}$.}
