...
\rput[t](1.2,-0.1){$x$}%
\uncover<2-15>{%
\rput[b](-0.832294, 1.85){$\alert<4>{P_2}$}%
\rput[bl](0.84, 1.3){$\alert<4>{P_1}$}%
...
\column{0.5\textwidth}
\uncover<2->{Split $[a,b]$ into $N$ equal segments via points $a=\theta_0 \leq \theta_1 \leq \dots \leq \theta_{N-1} \leq \theta_N=b$.} \uncover<3->{The length of each segment is $\Delta=\frac{b-a}{N}$.} \uncover<4->{Let $r_i=f(\theta_i)$. Then each $\theta_i$ gives a \alert<4>{point $P_i$} with polar coordinates $(\alert<5>{r_i},\alert<6>{\theta_i})$.}
\end{columns}
