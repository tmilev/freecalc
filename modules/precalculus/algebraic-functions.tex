...
\begin{frame}
\frametitle{Algebraic Functions}
\begin{emptyTheorem}[Algebraic Function]
...
\uncover<2->{{\footnotesize Outside of present course: function $f(x)$ = algebraic if it satisfies a polynomial equation with polynomial coefficients, i.e., $a_0(x) +a_1(x)f(x)+\dots +a_n(x) \left(f(x)\right)^n=0$ for some polynomials  $a_i(x)$.}}
\end{emptyTheorem}
\uncover<3->{
Examples.
