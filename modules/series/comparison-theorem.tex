...
\begin{itemize}
\item<2->  A $p$-series \uncover<3->{($\sum \frac{1}{n^p}$ \alert<handout:0| 3-4>{converges if \uncover<4->{$p>1$}} and \alert<handout:0| 5-6>{diverges if \uncover<6->{$p\leq 1$}})}
\item<2->  A geometric series \uncover<7->{($\sum a r^{n-1}$ \alert<handout:0| 7-8>{converges if \uncover<8->{$|r| <1$}} and \alert<handout:0| 9-10>{diverges if \uncover<10->{$|r| \geq 1$}})}
\end{itemize}
\end{frame}
