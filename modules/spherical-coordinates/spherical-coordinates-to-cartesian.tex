\begin{frame}
\frametitle{Transition Spherical - Rectangular coordinates}
\begin{columns}
\column{0.4\textwidth}
\psset{xunit=1cm, yunit=1cm}
\begin{pspicture}(-2.1,-2.5)(5,5.2)
\tiny
\renewcommand{\fcScreen}{[-1 -0.4 -0.25] -1}
\fcLineIIId[arrows=->]{[0 0 0]}{[5 0 0 ]}
\fcLineIIId[arrows=->]{[0 0 0]}{[0 5 0 ]}
\fcLineIIId[arrows=->]{[0 0 0]}{[0 0 5 ]}
\fcPutIIId[r]{[0 -0.15 0.15]}{\alertNoH{2,3,4}{$O$}}
\fcLineIIId[linecolor=gray]{[3 0 0]}{[3 3 0]}
\fcLineIIId[linecolor=gray]{[3 3 0]}{[0 3 0]}
\fcLineIIId{[0 0 0]}{[3 3 0]}
\fcLineIIId{[3 3 0]}{[3 3 3]}
\fcLineIIId{[0 0 0]}{[3 3 3]}
\fcLineIIId{[0 0 3]}{[3 3 3]}
\fcPutIIId[l]{[3.1 3.1 3.1]}{\alertNoH{4}{$P$}}
\fcPutIIId[r]{[1.5 -0.1 0]}{$\alertNoH{1}{x}$}
\fcPutIIId[t]{[3.1 3.1 0]}{\alertNoH{2,3}{$Q$}}
\uncover<2->{\fcPutIIId[l]{[1.4 1.5 0]}{\alertNoH{4,5,6}{$~~r$}}}
\uncover<2->{
\fcPutIIId[r]{[3 -0.1 0]}{\alertNoH{2,3}{$S$}}
\fcPolyLineIIId[linecolor=red]{[2.6 0 0 ] [2.6 0.4 0] [3 0.4 0]}
}
\uncover<2->{
\fcPutIIId[b]{[-0.1 1.5 0]}{$y$}
\fcPutIIId[t]{[3.1 1.5 0]}{\alertNoH{5}{$y$}}
}
\uncover<7->{\fcPutIIId[r]{[0 0 1.5]}{\alertNoH{7}{$z~~$}}}
\uncover<4->{
\fcPutIIId[r]{[0 -0.1 3]}{\alertNoH{4}{$T~$}}
\fcPutIIId[b]{[1.4 1.5 3]}{\alertNoH{4,6}{$~~r$}}
\fcPolyLineIIId[linecolor=red]{[2.6 2.6 0] [2.6 2.6 0.56569] [3 3 0.56569]}
\fcPolyLineIIId[linecolor=red]{[0.4 0.4 3] [0.4 0.4 3 0.56569 sub] [0 0 3 0.56569 sub]}
}
\fcPutIIId[r]{[1.7 1.7 1.7]}{\alertNoH{4,6,7}{$\rho~~$}}
\fcPutIIId[rt]{[0.8 0.8 1.3]}{\alertNoH{4,6,7}{$\phi~~$}}
\fcPutIIId[br]{[1.2 0.5 0]}{\alertNoH{3,5}{$\theta$}}
\fcCurveIIId[linecolor=red, arrows=->]{0}{54.74}{[t sin 45 cos mul 1.5 mul t sin 45 sin mul 1.5 mul t cos 1.5 mul ]} %
\fcCurveIIId[linecolor=red, arrows=->]{0}{45}{[t cos 1.5 mul t sin 1.5 mul 0]} %
\end{pspicture}
\column{0.6\textwidth}

From spherical to rectangular coords:
\[
\begin{array}{rcll|l}
\alertNoH{1-}{x}&\alertNoH{1}{=}& \uncover<3->{\alertNoH{3}{ \alertNoH{4}{r}\cos\theta}} &&\uncover<2->{\alertNoH{2,5}{ \text{use }\triangle SQO}} \\
\uncover<4->{&\alertNoH{0}{=} & \alertNoH{4}{\rho\sin\phi }\cos\theta &&\alertNoH{4,6,7}{ \text{use }\triangle OPT} }\\
\uncover<5->{\alertNoH{5}{y}&\alertNoH{0}{=}& \alertNoH{5}{\alertNoH{6}{r} \sin\theta } }\\
\uncover<6->{&\alertNoH{0}{=}& \alertNoH{6}{\rho\sin\phi }\sin\theta} \\
\uncover<7->{\alertNoH{7}{z}&\alertNoH{7}{=}& \alertNoH{7}{\rho\cos\phi }}
\end{array}
\]
\uncover<8->{
From rectangular to spherical coords:
$$\rho = \sqrt{x^2+y^2+z^2} \quad , \quad r = \sqrt{x^2+y^2}$$
$$\cos\phi = \frac{z}{\rho} \quad , \quad \sin\phi = \frac{r}{\rho}$$
$$\cos\theta = \frac{x}{r} \quad , \quad \sin\theta = \frac{y}{r}$$
}
\end{columns}
\end{frame}

