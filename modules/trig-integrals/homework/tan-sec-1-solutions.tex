...
\noindent 
$\begin{array}{rcll|l}
\displaystyle \int \sec x \text{d} x&=&\displaystyle \int \sec (2\arctan t) \text{d} (2\arctan t)&& \text{Set } x= 2\arctan t\\
&=&\displaystyle \int \frac{1}{\cos (2\arctan t)} \frac{2}{1+t^2}\text{d} t&& \text{Use } \cos (2 z)= \frac{1-\tan^2 z }{1+\tan^2 z}\\
&=&\displaystyle \int \frac{1}{\frac{1 -t^2}{1 +t^2}} \frac{ 2 }{ 1+t^2}\text{d} t\\
&=&\displaystyle \int \frac{2}{1-t^2}\text{d} t&&\text{part. fractions}\\
&=&\displaystyle \int \left(\frac{1}{1-t}+ \frac{1}{ 1+t} \right)\text{d} t\\
...
\ref{problemintsecxdx}. \textbf{Variant II.} This variant is based on the following observation. For an odd number $m>0 $, we studied a quick technique for integrating $\int \sin^n x \cos^m x \text{d} x $: namely, use the transformation $\cos x \text{d} x = \text{d} (\sin x)$ and change variables $u=\sin x $. This trick relies heavily on the fact that $m$ is odd (as we need to express the remaining even power of $\cos x$ via $\sin x$). However, the positivity of $m$ is not essential: by multiplying top and bottom by $\cos x$ we can make this technique work also for odd negative values of $m$. We illustrate the technique in the solution below.

$\begin{array}{rcll|l}
\displaystyle \int \sec x \text{d} x&=&\displaystyle \int \frac{1}{\cos x}\text{d} x\\
&=&\displaystyle \int \frac{\cos x}{\cos^2x}\text{d} x\\
&=&\displaystyle \int \frac{\text{d} (\sin x)}{1-\sin^2x}&&\text{Set }u=\sin x\\
&=&\displaystyle \int \frac{\text{d} u}{1-u^2}\\
&=&\displaystyle \int \frac{\text{d} u}{(1+u)(1-u)}&&\text{part. fractions}\\
&=&\displaystyle \int \left(\frac{\frac{1}{2}}{1+u} +\frac{ \frac{1}{ 2}}{1 -u} \right)\text{d} u \\
...
$\begin{array}{rcll|l}
\displaystyle \int \sec x \text{d} x&=&\displaystyle \int \sec x \frac{\sec x +\tan x }{\sec x+\tan x }\text{d} x  \\
&=& \displaystyle \int \frac{\sec^2 x+\sec x\tan x }{\sec x +\tan x }\text{d} x&&
\begin{array}{rcl}
\text{d} (\tan x )&=&\sec^2 x\text{d} x\\
\text{d} (\sec x)&=&\sec x\tan x \text{d} x
\end{array}\\
&=&\displaystyle  \int \frac{\text{d} (\sec x+\tan x) }{\sec x +\tan x} &&\text{Set }u=\sec x +\tan x\\
&=&\displaystyle \int \frac{\text{d} u}{u}\\
&=&\displaystyle \ln |u|+C\\
&=&\displaystyle \ln |\sec x +\tan x|+C\quad .
\end{array}
$
...
\noindent $
\begin{array}{rcll|l}
\displaystyle \int \sec^3 x \text{d} x&=&\displaystyle \int  \frac{1}{\cos^3 x} \text{d} x \\
&=&\displaystyle \int  \frac{\cos x}{\cos^4 x} \text{d} x  && \text{use }\text{d} (\sin x)=\cos x \text{d} x\\
&=&\displaystyle \int  \frac{1}{\cos^4 x} \text{d} (\sin x)&& \text{use }\cos^2x=1-\sin^2x \\
&=&\displaystyle \int  \frac{1}{\left(1-\sin^2x\right)^2} \text{d} (\sin x) &&\text{Set }\sin x=u\\
&=&\displaystyle \int  \frac{1}{\left(1-u^2\right)^2} \text{d} u&&\text{split in part. frac.}\\
&=&\displaystyle \int\left(\frac{\frac{1}{4}}{u +1}+\frac{\frac{1}{4}}{(u +1)^{2}}+\frac{-\frac{1}{4}}{u -1}+\frac{\frac{1}{4}}{(u -1)^{2}}  \right) \text{d} u\\
...
\noindent $
\begin{array}{rcll|l}
\displaystyle \int \sec^3 x \text{d} x&=&\displaystyle \int  \sec x \text{d} (\tan x) &&\text{int. by parts}\\
&=&\displaystyle  \sec x\tan x - \int \tan x \text{d} (\sec x)\\
&=&\displaystyle \sec x \tan x - \int \sec x \tan^2 x\text{d} x &&\tan^2 x =\sec^2x-1\\
&=&\displaystyle \sec x \tan x - \int\sec x (\sec^2x-1)\text{d} x\\
&=&\displaystyle \sec x \tan x - \int \sec^3 x \text{d} x+\int \sec x \text{d} x &&\text{Use Problem }\ref{problemintsecxdx}\\
&=&\displaystyle \sec x \tan x -\int \sec^3x \text{d} x+\ln |\sec x +\tan x|&& \begin{array}{l} + \int \sec^3x \text{d} x \\ 
\text{ to both sides}\end{array} \\
\displaystyle 2\int \sec^3x \text{d} x&=&\displaystyle \left(\sec x\tan x +\ln |\sec x+\tan x| \right) +C\\
\displaystyle \int \sec^3x \text{d} x&=&\displaystyle \frac{1}{2}\left(\sec x\tan x +\ln |\sec x+\tan x| \right) +K\quad .
\end{array}
$
}