...
\item $\fcv r_1=\langle x_1, y_1, z_1\rangle$, $u=\langle p,q,r\rangle$,
\item plane $\mathcal{P}: \alert<12>{\alert<4>{ax +by +cz} \alert<5>{ -d}=0}$.
\item Goal: find the intersection between line and plane.
\end{itemize}
\end{columns}
\uncover<2->{Let $P_0(\fcv r_0)$ be a point on the plane.} \uncover<3->{Then a point $P(\fcv r)$, $\fcv r=\langle x,y,z\rangle$ is on the plane if 
$
...
\uncover<12->{&=& \langle x_1, y_1, z_1 \rangle-\frac{\alert<12>{ ax_1 +by_1+cz_1-d}}{ap + bq +cr }\langle p,q, r\rangle}
\end{array}
\]
}
\end{frame}