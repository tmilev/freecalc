...
\newcommand{\fcLightSource}{[2 -1 2] 1\space} %Lightsource location, constant = brightness.
...
/y exch def
/x exch def
/vZeroProj vZero \fcProjectOntoScreen def
/dirVectProjOne vOne \fcProjectOntoScreen vZeroProj \fcVectorMinusVector def
/dirVectProjTwo vTwo \fcProjectOntoScreen vZeroProj \fcVectorMinusVector def
...
\newcommand{\fcFourBitsToHexStringCode}{
1 dict begin 
/theNum exch def
theNum 0 lt {/theNum 0 def}if
theNum 15 gt {/theNum 15 def}if
theNum 10 lt {theNum 1 string cvs}if
theNum 10 eq {(A)}if
theNum 11 eq {(B)}if
theNum 12 eq {(C)}if
theNum 13 eq {(D)}if
...
theByte 16 mod dup theByte exch sub 16 div cvi \fcFourBitsToHexStringCode exch \fcFourBitsToHexStringCode \fcConcatenate
end\space
}
...
\fcGetPointOnPlaneThatProjectsToXYCode \fcLightSource\space pop \fcDistance  1 exch div currentColor exch 256 mul \fcVectorTimesScalar \fcColorToColorStringCode
}
...
/numXiterations 2 def
/numYiterations 2 def
...
/Function <<
/FunctionType 0
/Order 3 %order=3 use cubic interpolation, whatever that means
...
(got to here) ==
pstack
numXiterations numYiterations mul \fcConcatenateMultiple 
%dup ==
%filter
%currentfile
%def
...
ifelse]
def
currentVisibleColor \fcArrayToStack setrgbcolorVirtual
0.5 setalpha
newpathVirtual
