...
Suppose $f$ is continuous on the closed interval $[a,b]$ and \alert<2,4,6,8>{let $N$ be any number between $f(a)$ and $f(b)$}, where $f(a) \neq f(b)$.  Then there \alert<3,5,7,9>{exists a number $c$} in $(a,b)$ \alert<3,5,7,9>{such that $f(c) = N$}.
\end{theorem}
\begin{center}

\begin{pspicture}(-0.5,-0.5)(6.2,4)
...
\rput[t](2, -0.1){\alert<9>{$c_1$}}
\rput[t](3, -0.1){\alert<9>{$c_2$}}
\rput[t](5, -0.1){\alert<9>{$c_3$}}
}
\end{pspicture}
%\ \only<handout:0| -1>{%
