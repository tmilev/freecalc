...
\begin{example}
Derive \alert<11>{the exponent rule $\left(e^x\right)'=e^x$} using \alert<4>{the power rule $(x^n)'=nx^{n-1}$}, the \alert<3>{\uncover<3->{ \alert<3>{infinite} {\color{gray!50} (both sides uniformly convergent)}} sum rule $(f_1+f_2\uncover<3->{ \alert<3>{ +f_3 +\dots}})' =f_1' +f_2' \uncover<3->{ \alert<3>{ +f_3' +\dots}}$} and the Calc II formula
\[
\alert<2,10>{e^x}=\alert<2,10>{1+x+\frac{x^2}{2!}+\frac{x^3}{3!}+\dots},
\]
where $n!=1*2*3*\dots*n$. \uncover<5->{We have that $\alert<5,9>{\frac{n}{\alert<6>{ n!}}} =\uncover<6->{ \frac{\alert<7>{n} }{ \alert<6>{ 1*2*\dots*(n-1) * \alert<7>{n}}}=}\uncover<7->{\frac{1}{\alert<8>{1*2*\dots*(n-1)}}=}\uncover<8->{\alert<9>{ \frac{1}{ \alert<8>{(n-1)!} }}} $.}
\[
\begin{array}{rcl}
