...
\begin{itemize}
\item<2-> Identify the \alertNoH{2}{lowest point(s), with $y$-coordinate $y=c$} and \alertNoH{3}{the topmost point(s), $y=d$}.
...
\begin{itemize}
\item<6-> horizontal lines $\alertNoH{6}{y=c}$ and $\alertNoH{6}{y=d}$
\item<7-> graphs of $\alertNoH{7}{x=f(y)}$ and $\alertNoH{7}{x=g(y)}$, with  $f,g \colon [c,d] \to \mathbb{R}$:
\end{itemize}
}
\uncover<8->{
\[
