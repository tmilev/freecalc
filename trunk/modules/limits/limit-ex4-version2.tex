...
\column{.4\textwidth}
\begin{itemize}
\item  Guess the value of $\lim\limits_{x\rightarrow 0}\sin\frac{\pi}{x}$.
\item<2->  Notice that $\sin\frac{\pi}{x}$ is not defined at $0$.  
\item<3->  It is defined at values near $0$.
\item<5->  We could guess that the limit is $0$. 
\item<6->  In this case, the guess is \alert<6>{wrong}.
\end{itemize}
\column{.6\textwidth}
\uncover<4->{
\[
