...
\column{.4\textwidth}
\begin{itemize}
\item  Guess the value of $\lim\limits_{x\rightarrow 0}\sin \left(\frac{\pi}{x}\right)$.
\item<2->  Notice that $\sin\left(\frac{\pi}{x}\right)$ is not defined at $0$.
\item<3->  It is defined for values of $x$ near $0$.
\item<5->  We may guess that the limit is $0$.
\item<6->  Such a guess would be \alert<6>{wrong}.
\end{itemize}
\column{.6\textwidth}
\uncover<4->{
\[
