...
\end{definition}
\only<handout:2|2>{If we don't require that the functions be \alert<handout:2|2>{continuous}, every set of points will be a curve and the definition would be pointless.}

\uncover<handout:3|3->{Informally, a curve image ``remembers'' only the points lying on the curve but forgets the ``speed'' with which each point was visited and ``how many times'' each point was visited.
}
\end{frame}