...
\end{pspicture}
\column{0.75\textwidth}
A curve $C$ is defined by \alertNoH{ 2}{$x = t^2, y = t^3 - 3t$}.
\end{columns}
\begin{enumerate}
\item Show $C$ traverses $(x,y)=(3,0)$ for two values of $t$; find the tangent slopes for both of these values.
\end{enumerate}
\begin{itemize}
\item<2-| alert@3-4>  $3 = \alertNoH{ 2}{x = t^2}$ \ if \ $t = $ \uncover<4->{$\pm \sqrt{3}$.}
\item<2-| alert@5-6>  $0 = \alertNoH{ 2}{y = t^3 - 3t} = t(t^2-3)$\  if \ $t = $ \uncover<6->{$0$\  or\  $\pm \sqrt{3}$.}
...
} \uncover<9->{= \frac{\alertNoH{ 10}{\uncover<10->{3t^2-3 }}}{\alertNoH{ 12}{\uncover<12->{2t }}}}$\uncover<12->{\quad .}
\item<13->
Plug in $t = \pm \sqrt{3}$:
$\displaystyle
\uncover<13->{%
...
& \uncover<2->{ = } &%
\displaystyle \uncover<2->{%
\frac{\frac{\diff}{\diff t}\left( \alertNoH{ 3-4}{\frac{\diff y}{\diff x}}\right)}{\alertNoH{ 5-6}{\frac{\diff x}{\diff t}}}%
}  \uncover<3->{ = }  \uncover<3->{%
\frac{\frac{\diff}{\diff t}\left( \alertNoH{ 3-4,7}{\uncover<4->{\frac{3t^2-3}{2t}}}\right)}{\alertNoH{ 5-6}{\uncover<6->{2t}}}%
}\\%
& \uncover<7->{ = } &\displaystyle
\uncover<7->{%
\frac{\alertNoH{ 8-9}{\frac{\diff}{\diff t}\left( \alertNoH{ 7}{\frac{3}{2}\left( t - \frac{1}{t}\right)}\right)}}{2t}%
}  \uncover<8->{ = }  \uncover<8->{%
\frac{\alertNoH{ 8-9}{\uncover<9->{\frac{3}{2} + \frac{3}{2t^2} }}}{2t}%
}\\%
