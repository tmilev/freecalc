...
\begin{itemize}
\item \alertNoH{5}{A curve parametrization is a function} $\fcv{r}: [a,b] \to \RR^2$ $\fcv{r}:[a,b]\to \RR^{3}$, or $\fcv{r}:[a,b]\to\RR^{n}  $ in general.
\item<2-> Input is scalar (parameter).
\item<3-> Output is (position) vector.
\item<4-> The image of $\fcv{r}$ is a \alertNoH{5}{ set of points} in $\RR^{n}$; we call that set \alertNoH{5}{curve image}.
\item<5-> The term ``curve'' is ambiguous and either means a \alertNoH{5}{ curve parametrization or a curve image}.
