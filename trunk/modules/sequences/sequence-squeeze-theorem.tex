...
If $\only<8->{\color{blue}}f(x)\color{black} \leq \only<10->{\color{red}} g(x)\color{black}\leq \only<9->{\color{darkgreen}} h(x)$ and $\lim_{x\to\infty} f(x) = L = \lim_{x\to\infty}{\color{darkgreen} h(x)}$, then $\lim_{x\to\infty}{\color{red} g(x)}=L$.
\end{theorem}
}
\end{frame}
