...
\begin{pspicture}(-0.4,-1.2)(1.6,1.5)
\tiny
...
\begin{itemize}
\item \alertNoH{2}{ $\fcv{u} \times \fcv{v}$ is perpendicular to both $\fcv{u}$ and $\textbf{v}$.}
\item The magnitude of $\fcv{u} \times \fcv{v}$ equals $|\fcv{u}| \alertNoH{3}{| \textbf{orth}_{\fcv u} \fcv v |}=|\textbf{u}| \alertNoH{3}{|\textbf{v}| \sin{\alpha}}$.
\item \alertNoH{4-12}{The direction of $\textbf{u} \times \textbf{v}$ is such that when viewed from the tip of $\fcv{u}\times \fcv v$, $\fcv v$ is counter-clockwise from $\fcv u$.}
\end{itemize}
\item If $\fcv{u}$, $\fcv{v}$ are colinear or zero then $\fcv u \times \fcv v=\fcv 0$.
\end{itemize}
\end{definition}
\end{columns}
\uncover<12->{\alertNoH{12}{There are a couple of hand rules to help figure out the direction of the cross product.}}
\end{frame}
